...
zips_withhospital_centroids = active_hospital_count.merge(zips_texas_centroids, on='ZIP_CD', how='left')
```
...
# correct crs for measuring distance
...
# create function to calculate the nearest distance
def calculate_nearest_distance(row, hospital_geometries):
    current_geom = row.geometry
    nearest_geom = nearest_points(current_geom, hospital_geometries.union_all())[1]
    return current_geom.distance(nearest_geom)

# create timer
start_time_sample = time.time()

# run function with the sample zipcodes
sampled_zips_texas['distance_meters'] = sampled_zips_texas.apply(
    calculate_nearest_distance, hospital_geometries=zips_withhospital_centroids['geometry'], axis=1
)
sampled_zips_texas['distance_miles'] = sampled_zips_texas['distance_meters'] * 0.000621371
...
print(sampled_zips_texas[['ZIP_CD', 'distance_miles']])
...
First Draft:
```{python}
# repeat for all texas zipcodes
start_time_full = time.time()
zips_texas_centroids['distance_meters'] = zips_texas_centroids.apply(
    calculate_nearest_distance, hospital_geometries=zips_withhospital_centroids['geometry'], axis=1
)
zips_texas_centroids['distance_miles'] = zips_texas_centroids['distance_meters'] * 0.000621371
end_time_full = time.time()

# time taken for all Texas zipcodes
print("Time taken for full calculation:", end_time_full - start_time_full, "seconds")

print(zips_texas_centroids[['ZIP_CD', 'distance_miles']].head(10))
```
